...
window.initSearch = function initSearch(resultsPerPage,searchTypes,searchCurrentSiteOnly,hideSearchForm,hideFacetsFilter,firstInit) {
    ...
    function renderCachedResults(append) {
      ...
      var resultHeader =eXo.ecm.WCMUtils.getBundle("unifiedSearch.label.Results" , eXo.env.portal.language)+ " "+ 1 +" "+ eXo.ecm.WCMUtils.getBundle("unifiedSearch.label.to" , eXo.env.portal.language)   +" "+ NUM_RESULTS_RENDERED + " "+eXo.ecm.WCMUtils.getBundle("unifiedSearch.label.for" , eXo.env.portal.language)   +"  <strong>" +  XSSUtils.sanitizeString($("#txtQuery").val()) + "<strong>";
